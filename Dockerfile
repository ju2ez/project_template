...
# and `pip install ray_tune_template` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "ray_tune_template", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n ray_tune_template -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "ray_tune_template", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "ray_tune_template", "YOUR_CONSOLE_SCRIPT"]
